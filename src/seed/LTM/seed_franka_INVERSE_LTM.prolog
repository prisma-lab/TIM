...
schema(iiwaAttach(Obj1,Obj2), [
	[hardSequence([iiwaGo(any,Obj2.pre),iiwaWait(w2),iiwaInsert(Obj1,Obj2.over),iiwaWait(w3),wsg50Release(Obj1),iiwaGo(Obj2.over,home),timer(on(Obj1,Obj2),true,0.1)]), 0, ["TRUE"] ] ], 
	[on(Obj1,Obj2)], []).
	
schema(iiwaDetach(Obj1,Obj2), [
	[hardSequence([iiwaGo(any,Obj1.pre),iiwaGo(Obj1.pre,Obj1),iiwaWait(w2),wsg50Grasp(Obj1),iiwaExtract(Obj1,Obj2.pre),iiwaWait(w3),iiwaGo(Obj2.pre,home),timer(Obj1.taken,true,0.1)]), 0, ["TRUE"] ] ], 
	[Obj1.taken], []).
...
schema(frankaOpen(R,Obj1,Frame), [], [frankaOpen(R,Obj1,Frame)], [] ).

schema(frankaClose(R,Obj1,Frame), [], [frankaClose(R,Obj1,Frame)], [] ).



% FRANKA tasks (abstract) -> INVERSE UC-CRF

% robot (R) picks object (Obj) from a pose (Pos)
%	NOTE: Pos must have a Pos.over defined
schema(frankaPick(R,Obj,Pos), [
	[hardSequence([
		frankaGo(R,Pos.over),
		frankaGo(R,Pos),
		frankaClose(R,Obj,Pos),
		frankaGo(R,Pos.over),
		timer(taken(Obj),true,0.1)
	]), 0, ["TRUE"] ] ], 
	[taken(Obj)], []).

% robot (R) places object (Obj) to a pose (Pos)
%	NOTE: Pos must have a Pos.over defined
schema(frankaPlace(R,Obj,Pos), [
	[hardSequence([
		frankaGo(R,Pos.over),
		frankaGo(R,Pos),
		frankaOpen(R,Obj,Pos),
		frankaGo(R,Pos.over),
		timer(placed(Obj,Pos),true,0.1)
	]), 0, ["TRUE"] ] ], 
	[placed(Obj,Pos)], []).

% robot (R) positions object (Obj) from a pose (FromPos) to another pose (ToPos)
%	NOTE: Pos must have a Pos.over defined
schema(position(R,Obj,FromPos,ToPos), [
	[hardSequence([
		frankaGo(R,home),
...
%Assembly connection bar between 2 battery modules in the center of the battery
schema(assembly(R,connection_bar), [
	[hardSequence([
		% remove protective hoods omitted for brevity
		position(R,connection_bar,kit1_1,battery_center),
		position(R,screw,kit1_2,battery_module_1),
		position(R,screw,kit1_3,battery_module_2) ]), 0, ["TRUE"] ],
	[user_monitor(tighten(screw,battery_module_1)), 0, [positioned(screw,battery_module_1)] ],
	[user_monitor(tighten(screw,battery_module_2)), 0, [positioned(screw,battery_module_2)] ],
