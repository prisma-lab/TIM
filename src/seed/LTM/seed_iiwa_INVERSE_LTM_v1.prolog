...
inverse(timer(X,Y,Z),timer(X,Y,Z)).
...
invert(S_dir, -S_dir) :- schema(S_dir, Sub_dir, _, _), apply_inversion(Sub_dir, Sub_inv), write("NEW: "), write(schema(-S_dir,Sub_inv,[],[])).
