...
% MANUAL INVERSION OF iiwaStack(brick22,brick21)  
schema(-iiwaPick(brick22),
	[[hardSequence([
		timer(brick22.taken,false,0.1),
		iiwaGo(home,rnd), %iiwaGo(home,brick22),
		wsg50Release(brick22),
		iiwaWait(w1),
		iiwaGo(brick22,brick22.pre),
		iiwaGo(brick22.pre,home)]),0,["TRUE"]]],[iiwaGo(brick22.pre, home).done],[]).

schema(-iiwaAttach(brick22,brick21),
	[[hardSequence([
		timer(on(brick22,brick21),false,0.1), 
		iiwaGo(home,brick21.over),
		wsg50Grasp(brick22),
		iiwaWait(w3),
		iiwaExtract(brick22,brick21.over),
		iiwaWait(w2),
		iiwaGo(brick21.pre,home)]),0,["TRUE"]]],[iiwaGo(brick21.pre, home).done],[]).


schema(-iiwaStack(brick22,brick21),
	[[hardSequence([
		%iiwaSimpleDetach(brick22,brick21), 
		-iiwaAttach(brick22,brick21),
		iiwaWait(w0),
		-iiwaPick(brick22)]),0,["TRUE"]]],[iiwaGo(brick22.pre, home).done],[]).


%MANUALLY DESIGNED SIMPLE-DETACHING TASK
%	this is intended as an example that simple-detaching (without extraction) is NOT working!
schema(iiwaSimpleDetach(brick22,brick21),
	[[hardSequence([
		timer(on(brick22,brick21),false,0.1), 
		iiwaGo(home,brick21.over),
		wsg50Grasp(brick22),
